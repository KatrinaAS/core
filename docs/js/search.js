...
// attach itself to a search form and to a search results pane identified by the
...
// 'search' that will trigger searching for a series of results, were the
// data-search-results pane is expected to have a direct UL child that will be
...
// the class 'phpdocumentor-search--active', indicating search is ready. At this
...
var Search = (function() {
  ...
  function close() {
    ...
    var searchField =
        document.querySelector('[data-search-form] input[type="search"]');
    searchField.blur();
  }
  ...
  function search(event) {
    ...
    results.forEach(function(result) {
      ...
      entry.innerHTML += '<h3><a href="' + document.baseURI + result.url +
                         '">' + result.name + "</a></h3>\n";
  ...
  function init() {
    ...
    var closeButton =
        document.querySelector('.phpdocumentor-search-results__close');
...
window.addEventListener('load', function() { Search.init(); });
